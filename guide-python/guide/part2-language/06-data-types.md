...
#### tuple vs set：为什么两个都要？

看起来都是"一组东西"，但解决完全不同的问题：

| | tuple | set |
|------|-------|-----|
| **本质** | 不可变的列表 | 数学意义上的集合 |
| **有序？** | ✅ 按索引访问 `t[0]` | ❌ 无序，不能索引 |
| **可重复？** | ✅ `(1, 1, 2)` | ❌ 自动去重 `{1, 2}` |
| **可变？** | ❌ 不可变 | ✅ `add` / `remove` |
| **字面量** | `(1, 2, 3)` | `{1, 2, 3}` |
| **典型操作** | 索引、切片、解包 | `in`、`&`、`|`、`-` |
...
point = (10, 20)          # 坐标
x, y = point              # 解包
("user", 1)               # 字典 key（不可变所以能做 key）

# set — 用于"去重 + 快速查找 + 集合运算"
set([1, 1, 2, 2, 3])      # {1, 2, 3} — 去重
"admin" in users          # O(1) 成员判断，比 list 快得多
a & b                     # 交集
a | b                     # 并集
```

> set 答的是"在不在里面"，tuple 答的是"第几个是什么"。两个问题不同，两个类型各自存在。
